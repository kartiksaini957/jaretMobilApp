...
/// Primitives that let the LightSignal field be a faithful, copy-paste port
/// of the CSS in `LightSignal Visual Foundation v2` — CSS gradient angles and
/// `transparent` stops both need explicit translation to reach Flutter
/// unchanged.
class LsCss {
  LsCss._();
...
///
/// Flutter's [BoxShadow] only draws outer shadows, so inner glass highlights
/// are modelled here and drawn by [LsInsetShadowPainter].
@immutable
class LsInsetShadow {
  const LsInsetShadow({
...
/// Paints a gradient fill plus any [LsInsetShadow]s inside a rounded rect.
///
/// The inset model mirrors CSS exactly: take the border box, shrink it by the
...
class LsInsetShadowPainter extends CustomPainter {
  const LsInsetShadowPainter({
    ...
    this.shadows = const <LsInsetShadow>[],
  });

  final BorderRadius borderRadius;

  /// Fill gradients in CSS order — the first entry is the topmost layer.
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rrect = borderRadius.toRRect(rect).scaleRadii();
  ...
  void _paintShadow(Canvas canvas, RRect rrect, LsInsetShadow shadow) {
    final hole =
        rrect.deflate(shadow.spread).shift(shadow.offset).scaleRadii();
