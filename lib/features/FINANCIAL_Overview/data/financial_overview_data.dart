...
MetricTone _toneFromStatus(String status) {
  switch (status) {
    case 'above_average':
    case 'top_tier':
      return MetricTone.good;
    case 'below_average':
      return MetricTone.bad;
    default:
...
ImpactTone _impactToneFromValue(String impactValue) {
  final v = impactValue.trim();
  if (v.startsWith('+')) return ImpactTone.positive;
  if (v.startsWith('-')) return ImpactTone.negative;
  return ImpactTone.neutral;
}

List<double> _normalizedTrend(List<double> trend) {
  if (trend.isEmpty) return const [0.5, 0.5, 0.5, 0.5, 0.5];
  final maxVal = trend.reduce((a, b) => a > b ? a : b).abs();
  if (maxVal == 0) return trend.map((_) => 0.5).toList();
  return trend.map((v) => (v.abs() / maxVal).clamp(0.05, 1.0)).toList();
}

List<MetricDetail> metricDetailsFromKpiTiles(List<KpiTile> tiles) {
  return tiles.map((tile) {
    return MetricDetail(
      label: tile.label,
      value: tile.value,
      statusLabel: _statusLabel(tile.status),
      tone: _toneFromStatus(tile.status),
      trendLabel: tile.changeIndicator,
      chartValues: _normalizedTrend(tile.trend),
      chartCaption: '${tile.label} · recent periods',
      vsLastMonthBody: tile.verdict,
      vsLastMonthDeltaLabel: tile.changeIndicator,
      hasPeerComparison: false,
      drivingFactors: tile.drivers
          .map(
            (d) => DrivingFactor(
              title: d.headline,
              subtitle: d.category,
              impact: d.impactValue,
              tone: _impactToneFromValue(d.impactValue),
            ),
          )
          .toList(),
      ...
      askAiTranscript: [
        AskAiMessage(
          fromAi: true,
          text:
              'Scoped to ${tile.label} — ask anything and the answer '
              "uses this month's numbers.",
        ),
      ],
    );
  }).toList();
