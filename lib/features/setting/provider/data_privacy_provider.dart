...
class DataPrivacyController extends Notifier<DataPrivacyState> {
  @override
  DataPrivacyState build() {
    Future.microtask(_load);
    return const DataPrivacyState();
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final data = await ApiService().getDataPrivacySettings(accessToken: token);
      ...
      final message = e is ApiException ? e.message : 'Could not load privacy settings.';
  ...
  void setPeerBenchmarking(bool value) {
    ...
    _patch({'peer_benchmarking': value}).then((_) {
      if (state.error != null) state = state.copyWith(peerBenchmarking: previous);
    });
  }
  ...
  void setPhotoPermissions(bool value) {
    ...
    _patch({'photo_permissions': state.photoPermissionsJson}).then((_) {
      if (state.error != null) state = state.copyWith(photoPermissions: previous);
    });
  }
  ...
  void setGoogleBusinessProfile(bool value) {
    ...
    _patch({'photo_permissions': state.photoPermissionsJson}).then((_) {
      if (state.error != null) state = state.copyWith(googleBusinessProfile: previous);
    });
  }
